...
const BulkAssign: React.FC = () => {
  ...
  return (
    <div style={{ background: '#23263a', borderRadius: 12, padding: 24, marginBottom: 24 }}>
      <h3 style={{ color: '#4ea8de', marginBottom: 12 }}>Bulk Assign Roles & Departments</h3>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        <select
          value={selectedUserId}
          onChange={e => setSelectedUserId(e.target.value)}
          style={{ padding: '6px 12px', borderRadius: 6, background: '#181a20', color: '#f3f4f6', border: '1px solid #35374a', minWidth: 120 }}
        >
        ...
        <select
          ...
          style={{ padding: '6px 12px', borderRadius: 6, background: '#181a20', color: '#f3f4f6', border: '1px solid #35374a', minWidth: 120, height: 80 }}
        >
          {seedRoles.map(r => (
        ...
        <button
          className="icon-btn"
          style={{ fontSize: '1rem', padding: '6px 16px', background: '#00adb5', color: '#23263a', borderRadius: 6, border: 'none', fontWeight: 600 }}
          disabled={!selectedUserId || (selectedRoles.length === 0 && selectedDepartments.length === 0)}
          onClick={handleAssign}
        >Assign</button>
      </div>
      <div style={{ marginTop: 16, color: '#f3f4f6', fontSize: '0.98rem' }}>
        <b>Current Assignments:</b>
        <ul style={{ marginTop: 8 }}>
          {users.map(u => (
            <li key={u.userId}>
              <span style={{ color: '#4ea8de' }}>{u.firstName} {u.lastName} ({u.userId}):</span>
